...
`ADMIN_ENABLED=true` 时，登录管理端后在“账号”页点击“授权账号”。服务端会先登记一个绑定当前会话的授权任务，再生成一次性 Codebuff 授权链接；在新标签页完成你自己的账号登录后，服务端自动轮询并把 Token 加密写入 SQLite，浏览器和审计日志不会收到 Token。若生成链接时页面被关闭，重新打开“授权账号”会继续同一个未完成任务，不会重复创建授权。

严格代理模式下，授权完成的账号会先以**停用**状态保存。为该账号填写 HTTP/HTTPS/SOCKS5 代理并启用后，才会进入请求账号池。授权任务状态包含“生成中、等待授权、保存中、已完成/已取消/已过期”；取消、登出或会话失效会终止未完成任务。授权链接有效期很短，只能由当前管理员会话使用；请不要截图、转发或在明文 HTTP 管理端使用。公网管理端应先配置 HTTPS。
