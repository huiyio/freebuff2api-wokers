...
截至 2026-08-16，GHCR Package 已验证为 Public，可直接拉取。`1.8.9-admin.4` 的 Actions Run 和多架构 digest 会在不可变标签构建完成后记录到 [`DOCKER.md`](DOCKER.md)；如果后续可见性改变，私有包才需要 `read:packages` PAT 登录，且不要把 PAT 写入配置或日志。
